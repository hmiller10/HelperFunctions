...
Describe "Test-MyNetConnection" {
	...
	Context "Test parameters" {
		It "Should have verified parameters" {
			Get-Command Test-MyNetConnection -Server $ServerIP -Port $Port -Module HelperFunctions -CommandType Function | Should -HaveParameter -Parameter ServerName -Type System.String -Because "The function requires a destination to test."
